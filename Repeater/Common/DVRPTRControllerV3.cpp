...
wxArrayString CDVRPTRControllerV3::getDevices()
{
#if defined(__WINDOWS__)
...
CDVRPTRControllerV3::CDVRPTRControllerV3(const wxString& port, const wxString& path, bool txInvert, unsigned int modLevel, bool duplex, const wxString& callsign) :
...
CDVRPTRControllerV3::CDVRPTRControllerV3(const wxString& address, unsigned int port, bool txInvert, unsigned int modLevel, bool duplex, const wxString& callsign) :
wxThread(wxTHREAD_JOINABLE),
...
CDVRPTRControllerV3::~CDVRPTRControllerV3()
{
	delete m_usb;
	delete m_network;
...
void* CDVRPTRControllerV3::Entry()
{
...
void CDVRPTRControllerV3::purgeRX()
{
...
void CDVRPTRControllerV3::purgeTX()
{
	wxMutexLocker locker(m_mutex);
...
DATA_QUEUE_TYPE CDVRPTRControllerV3::readQueue(unsigned char* data, unsigned int& length)
{
	wxMutexLocker locker(m_mutex);
...
bool CDVRPTRControllerV3::writeHeader(const CHeaderData& header)
{
...
bool CDVRPTRControllerV3::writeData(const unsigned char* data, unsigned int length, bool end)
{
...
bool CDVRPTRControllerV3::getPTT() const
{
	return m_ptt;
}

bool CDVRPTRControllerV3::hasSpace()
{
	wxMutexLocker locker(m_mutex);
...
void CDVRPTRControllerV3::close()
{
	m_stopped = true;
...
bool CDVRPTRControllerV3::getSerial()
{
	unsigned char buffer[105U];
...
bool CDVRPTRControllerV3::getSpace()
{
...
bool CDVRPTRControllerV3::setConfig()
{
	unsigned char buffer[105U];
...
RESP_TYPE_V3 CDVRPTRControllerV3::getResponse(unsigned char *buffer, unsigned int& length)
{
...
wxString CDVRPTRControllerV3::getPath() const
{
	return m_usbPath;
}

bool CDVRPTRControllerV3::findPort()
{
...
bool CDVRPTRControllerV3::findPath()
{
	if (m_connection != CT_USB)
		return true;
...
bool CDVRPTRControllerV3::findModem()
{
	closeModem();
...
bool CDVRPTRControllerV3::openModem()
{
	bool ret = false;
...
int CDVRPTRControllerV3::readModem(unsigned char* buffer, unsigned int length)
{
	switch (m_connection) {
		case CT_USB:
...
bool CDVRPTRControllerV3::writeModem(const unsigned char* buffer, unsigned int length)
{
	switch (m_connection) {
		case CT_USB:
...
void CDVRPTRControllerV3::closeModem()
{
	switch (m_connection) {
		case CT_USB:
			return m_usb->close();
