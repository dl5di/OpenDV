...
#include "DStarRepeaterWindows.h"
...
const char*       NAME_PARAM = "Repeater Name";
const char* NOLOGGING_SWITCH = "nolog";
const char*    LOGDIR_OPTION = "logdir";
const char*  AUDIODIR_OPTION = "audiodir";

static CDStarRepeaterWindows* m_repeater = NULL;
...
int main(int argc, char** argv)
{
	...
	m_repeater = new CDStarRepeaterWindows(nolog, logDir, audioDir, name);
...
CDStarRepeaterWindows::CDStarRepeaterWindows(bool nolog, const std::string& logDir, const std::string& audioDir, const std::string& name) :
m_name(name),
m_nolog(nolog),
m_logDir(logDir),
m_confDir(confDir),
...
CDStarRepeaterWindows::~CDStarRepeaterWindows()
{
}

bool CDStarRepeaterWindows::init()
{
	if (!m_nolog) {
		std::string logBaseName = LOG_BASE_NAME;
		if (!m_name.IsEmpty()) {
...
void CDStarRepeaterWindows::run()
{
	m_thread->run();
...
bool CDStarRepeaterWindows::createThread()
{
	...
	std::string gatewayAddress, localAddress, name;
	...
	bool enabled;
	std::string rpt1Callsign, rpt2Callsign;
	std::string shutdown, startup;
	std::string status1, status2, status3, status4, status5;
	std::string command1, command1Line, command2, command2Line;
	std::string command3, command3Line, command4, command4Line;
	std::string command5, command5Line, command6, command6Line;
...
void CDStarRepeaterWindows::kill()
{
