...
const unsigned char DV3000_REQ_RESETSOFTCFG[]  = {DV3000_START_BYTE, 0x00U, 0x07U, DV3000_TYPE_CONTROL, DV3000_CONTROL_RESETSOFTCFG, 0x05U, 0x00U, 0x03U, 0x07U, 0x00U, 0x07U};
const unsigned int DV3000_REQ_RESETSOFTCFG_LEN = 11U;
...
CDVMEGAAMBEController::~CDVMEGAAMBEController()
{
}

bool CDVMEGAAMBEController::open()
{
...
void CDVMEGAAMBEController::encodeIn(const wxFloat32* audio, unsigned int length)
{
	wxASSERT(audio != NULL);
...
bool CDVMEGAAMBEController::encodeOut(unsigned char* ambe, unsigned int length)
{
	wxASSERT(ambe != NULL);
...
void CDVMEGAAMBEController::decodeIn(const unsigned char* ambe, unsigned int length)
{
	wxASSERT(ambe != NULL);
	wxASSERT(length == VOICE_FRAME_LENGTH_BYTES);
...
bool CDVMEGAAMBEController::decodeOut(wxFloat32* audio, unsigned int length)
{
	wxASSERT(audio != NULL);
	wxASSERT(length == DSTAR_AUDIO_BLOCK_SIZE);
...
void CDVMEGAAMBEController::close()
{
	m_serial.close();
}

CDVMEGAAMBEController::RESP_TYPE CDVMEGAAMBEController::getResponse(unsigned char* buffer, unsigned int length)
{
