...
m_dataBuffer(VOICE_FRAME_LENGTH_BYTES * 30U),
m_killed(false)
{
}
...
bool CDVToolReaderThread::setReader(CDVTOOLFileReader* reader)
{
	...
	if (m_dongle == NULL || m_soundcard == NULL) {
		wxLogError(wxT("No DV-Dongle and/or Soundcard has been set"));
		return false;
	}
...
void CDVToolReaderThread::setDVDongle(CDVDongleController* dongle)
{
	...
	m_dongle = new CDVDongleThread(dongle, &m_audioBuffer, &m_dataBuffer);
}
